// out/dianxian/kl/main.js — 车外电显 KL variant
importPackage(java.awt);
include(Resources.id("mtr:buslcds/lib/dianxian_util.js"));

var _KL_OUT_SIZE = [1600, 762];
var _KL_OUT_PANEL_H = 200;
var _KL_NUM_FONT = Resources.readFont(Resources.id("mtr:buslcds/font/kl/cepainum.otf"));

function getKlLedColor(cfg) {
    if (cfg && cfg.color === "red") return new Color(16727080);
    return new Color(16762940); // orange default
}
...
function drawOutKl(g, cfg, info, panelW, panelH) {
    ...
    var font = KL_LED_FONT.deriveFont(Font.PLAIN, fontSize);
    ...
    var sbKey = "out-kl-" + (_langState.cur || "cn");
    var sbX, sbW;
...
function drawOutKlFrontTail(g, cfg, info, veh, panelW, panelH, role) {
    var LED = getKlLedColor(cfg);
    g.setColor(Color.BLACK);
    g.fillRect(0, 0, panelW, panelH);
...
MODULE_IMPL = {
    ...
    renderOne: function(displays, train, rp, ap) {
        ...
        if (veh && (veh.state === "left" || veh.state === "right" || veh.state === "brake")
            ...
            print("[OUT-KL-DBG] veh.state=" + veh.state + " turnPhase=" + veh.turnPhase + " turnSide=" + veh.turnSide + " brakeBlink=" + veh.brakeBlink);
        }
        for (var i = 0; i < 3; i++) {
            var slotName = "out-dianxian-kl-" + i;
            var g = displays.graphicsFor(slotName);
            var role = roles[i];
            if (role === "front" || role === "tail") {
                drawOutKlFrontTail(g, cfg, info, veh, 1600, 200, role);
            } else {
                drawOutKl(g, cfg, info, 1600, 200);
            }
        }
        displays.upload();
    }
};
if (_MTR_LOG) print("[MTR] out_dianxian/kl main loaded");
