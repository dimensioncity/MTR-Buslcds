...
function DrawLeftPanel(g, startName, endName, attrs, direction) {
    if (_MTR_LOG) print("[MTR] DrawLeftPanel called, start=" + startName + ", end=" + endName);
    ...
    if (logoImg != null) {
        var bgW = 280, bgH = Math.round(logoImg.getHeight() * bgW / logoImg.getWidth());
        var bgX = (MAP_LEFT_W - bgW) / 2;
        var bgY = (400 - bgH) / 2;
        g.setComposite(java.awt.AlphaComposite.getInstance(java.awt.AlphaComposite.SRC_OVER, 0.25));
        g.drawImage(logoImg, bgX, bgY, bgW, bgH, null);
        g.setComposite(java.awt.AlphaComposite.getInstance(java.awt.AlphaComposite.SRC_OVER, 1.0));
    }

    // 顶栏 logo + "浦东公交"
    if (logoImg != null) {
        var logoTopSize = 36;
        var logoTopH = Math.round(logoImg.getHeight() * logoTopSize / logoImg.getWidth());
        g.drawImage(logoImg, 16, 14, logoTopSize, logoTopH, null);
    }
    g.setColor(MAP_LEFT_WHITE);
    g.setFont(COMPANY_FONT.deriveFont(Font.BOLD, 28));
    ...
    CentreText(g, startName || "\u2014", MAP_LEFT_W / 2, 92, MAP_LEFT_W - 40);

    var firstLast = direction == "dn" ? MAP_DEFAULT_FIRST_LAST_DN : MAP_DEFAULT_FIRST_LAST_UP;
    g.setColor(MAP_LEFT_WHITE);
    g.setFont(SERIF.deriveFont(Font.PLAIN, 18));
    CentreText(g, firstLast, MAP_LEFT_W / 2, 118, MAP_LEFT_W - 40);

    g.setColor(Color.decode("#3A7BC8"));
    g.fillRect(20, 130, MAP_LEFT_W - 40, 1);

    g.setColor(MAP_LEFT_WHITE);
    g.setFont(SERIF.deriveFont(Font.BOLD, 32));
    CentreText(g, endName || "\u2014", MAP_LEFT_W / 2, 202, MAP_LEFT_W - 40);

    var firstLast2 = direction == "dn" ? MAP_DEFAULT_FIRST_LAST_UP : MAP_DEFAULT_FIRST_LAST_DN;
    g.setColor(MAP_LEFT_WHITE);
    g.setFont(SERIF.deriveFont(Font.PLAIN, 18));
    CentreText(g, firstLast2, MAP_LEFT_W / 2, 228, MAP_LEFT_W - 40);

    g.setColor(Color.decode("#3A7BC8"));
    g.fillRect(20, 240, MAP_LEFT_W - 40, 1);

    var fareStr = attrs ? attrs["fare"] : null;
    var fare = parseFare(fareStr);
    g.setColor(MAP_LEFT_WHITE);
    g.setFont(SERIF.deriveFont(Font.PLAIN, 20));
    CentreText(g, fare.label, MAP_LEFT_W / 2, 294, MAP_LEFT_W - 40);

    var hotlineStr = attrs ? attrs["hotline"] : null;
    if (!hotlineStr || String(hotlineStr).trim().length == 0) hotlineStr = "服务热线: 50216666";
    g.setColor(MAP_LEFT_WHITE);
    g.setFont(SERIF.deriveFont(Font.PLAIN, 18));
    CentreText(g, String(hotlineStr).trim(), MAP_LEFT_W / 2, 324, MAP_LEFT_W - 40);
}
...
function DrawRightMap(g, stations, routeName) {
    ...
    g.setFont(SERIF.deriveFont(Font.BOLD, 60));
    CentreText(g, lineName + "站级示意图", MAP_RIGHT_X + MAP_RIGHT_W / 2, 80, MAP_RIGHT_W - 40);
    var LINE_Y = 120;
    ...
    for (var i = 0; i < n; i++) {
        ...
        if (st.type == "up") {
            ...
            g.drawString("上", cx - g.getFontMetrics().stringWidth("上") / 2, noteY);
        } else if (st.type == "dn") {
            g.setColor(MAP_DN_ONLY_GREEN);
            g.setFont(SERIF.deriveFont(Font.BOLD, noteCharSize));
            g.drawString("下", cx - g.getFontMetrics().stringWidth("下") / 2, noteY);
        }
    }
    // 方向箭头 (右区底部两端)
    var arrowY = 380;
    ...
    g.drawLine(leftX, arrowY, leftX + arrowSize * 2, arrowY);
    var redPoly = new java.awt.Polygon();
    redPoly.addPoint(leftX + arrowSize * 2, arrowY);
    redPoly.addPoint(leftX + arrowSize * 2 - arrowSize, Math.round(arrowY - arrowSize * 0.7));
    redPoly.addPoint(leftX + arrowSize * 2 - arrowSize, Math.round(arrowY + arrowSize * 0.7));
    g.fillPolygon(redPoly);
    // 右端: 绿箭头朝左 (下行方向)
    g.setColor(MAP_DN_ONLY_GREEN);
    g.setStroke(new BasicStroke(3));
    g.drawLine(rightX - arrowSize * 2, arrowY, rightX, arrowY);
    var greenPoly = new java.awt.Polygon();
    greenPoly.addPoint(rightX - arrowSize * 2, arrowY);
    greenPoly.addPoint(rightX - arrowSize * 2 + arrowSize, Math.round(arrowY - arrowSize * 0.7));
    greenPoly.addPoint(rightX - arrowSize * 2 + arrowSize, Math.round(arrowY + arrowSize * 0.7));
